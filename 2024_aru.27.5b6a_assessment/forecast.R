...
tac       <- FLQuant(21198, dimnames=list(age='all', year=iy), units="tonnes")
ImYC      <- FLQuant(17967, dimnames=list(age='all', year=iy), units="tonnes")
tacadvice <- FLQuant(17695, dimnames=list(age='all', year=iy), units="tonnes")
#-----------------------
# Deterministic run
#-----------------------
...
library(doParallel)
ncores <- ifelse(n<detectCores()-4,n,detectCores()-4)
